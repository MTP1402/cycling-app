"""
Cycling Coach – FastAPI Backend
================================
Handles user accounts, FIT file uploads, dashboard generation,
and AI coaching summaries.
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
DATABASE_URL   = os.environ.get("DATABASE_URL", "postgresql://postgres:zmqghZEJGkcgnGukasHvgntjfTNUVcyu@postgres.railway.internal:5432/railway")
...
# ── FIT parser (inline, no file dependency) ───────────────────────────────────
def parse_fit_bytes(data: bytes) -> dict:
    """Parse FIT file bytes and return ride metrics."""
    try:
        import fitparse
        import tempfile
        ...
        raise HTTPException(status_code=400, detail=f"Could not parse FIT file: {e}")
...
def register(email: str = Form(...), name: str = Form(...), password: str = Form(...)):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO users (email, name, password, token) VALUES (%s, %s, %s, %s) RETURNING id",
...
async def upload_fit(
    ...
    cur  = conn.cursor()
    cur.execute("""
        INSERT INTO rides (user_id, ride_date, name, dist_mi, duration_h,
            avg_power, norm_power, avg_hr, max_hr, avg_cadence, max_cadence,
            p5, p15, p30, temp_c, notes)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        RETURNING id
    """, (
        user['id'], metrics['ride_date'], metrics['name'],
        metrics['dist_mi'], metrics['duration_h'],
        metrics['avg_power'], metrics['norm_power'],
        metrics['avg_hr'], metrics['max_hr'],
        metrics['avg_cadence'], metrics['max_cadence'],
        metrics['p5'], metrics['p15'], metrics['p30'],
        metrics['temp_c'], notes
    ))
...
def get_rides(user: dict = Depends(get_current_user)):
    ...
    cur.execute("""
        ...
        ORDER BY ride_date DESC LIMIT 100
    """, (user['id'],))
    rides = cur.fetchall()
    cur.close()
    conn.close()
    return {"rides": [dict(r) for r in rides]}
...
def get_dashboard(user: dict = Depends(get_current_user)):
    ...
    cur.close()
    conn.close()
    html = build_dashboard_html(rides, user['name'])
    return HTMLResponse(content=html)

async def get_coaching_summary(user: dict, metrics: dict) -> str:
    ...
    try:
        ...
        cur.execute("""
            SELECT * FROM rides WHERE user_id = %s
            ORDER BY ride_date DESC LIMIT 10
        """, (user['id'],))
        ...
        context = f"Rider: {user['name']}\n"
        if notes:
            context += f"Personal notes: {'; '.join(notes)}\n"
        if recent:
            context += f"Recent rides (last 10): avg HR trend {[r['avg_hr'] for r in recent]}, avg power trend {[r['avg_power'] for r in recent]}\n"

        prompt = f"""{context}
Latest ride metrics:
- Date: {metrics['ride_date']}
- Distance: {metrics['dist_mi']} miles
- Duration: {metrics['duration_h']} hours
- Avg power: {metrics['avg_power']}W, Normalized power: {metrics['norm_power']}W
- Avg HR: {metrics['avg_hr']} bpm, Max HR: {metrics['max_hr']} bpm
- Avg cadence: {metrics['avg_cadence']} rpm
- 5s peak: {metrics['p5']}W, 15s peak: {metrics['p15']}W, 30s peak: {metrics['p30']}W
- Temperature: {metrics['temp_c']}°C

Give a brief 3-4 sentence coaching assessment of this ride. Be direct and specific."""
